% ============================================================
%  PayNexus — Relatório Técnico EGS
%  Grupo 3, DETI, Universidade de Aveiro — Junho 2026
% ============================================================
\documentclass[12pt,a4paper]{article}

% ----- Codificação e língua -----
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage[portuguese]{babel}

% ----- Layout -----
\usepackage[top=2.5cm, bottom=2.5cm, left=2.5cm, right=2.5cm]{geometry}
%\usepackage{microtype}
\usepackage{parskip}
\setlength{\parskip}{6pt}

% ----- Tabelas -----
\usepackage{booktabs}
\usepackage{tabularx}
\usepackage{longtable}
\usepackage{array}
\usepackage{colortbl}
\newcolumntype{L}[1]{>{\raggedright\arraybackslash}p{#1}}
\newcolumntype{C}[1]{>{\centering\arraybackslash}p{#1}}
% Separador de linha cinza claro entre linhas de dados
\newcommand{\rowsep}{\arrayrulecolor{gray!30}\hline\arrayrulecolor{black}}
\setlength{\extrarowheight}{2pt}

% ----- Figuras e floats -----
\usepackage{graphicx}
\graphicspath{{imgs/}}
\usepackage{float}
\usepackage{caption}
\usepackage[dvipsnames]{xcolor}

% ----- Código -----
\usepackage{listings}
\lstset{
  basicstyle=\ttfamily\small,
  frame=single,
  breaklines=true,
  backgroundcolor=\color{gray!8},
  rulecolor=\color{gray!40},
  numbers=left,
  numberstyle=\tiny\color{gray},
  xleftmargin=1.5em,
  framexleftmargin=1.5em,
}

% ----- Listas -----
\usepackage{enumitem}

% ----- Cabeçalhos -----
\usepackage{fancyhdr}
\setlength{\headheight}{14.5pt}
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\textit{PayNexus — EGS Platform}}
\fancyhead[R]{\small\textit{Grupo 3 · UA/DETI · 2026}}
\fancyfoot[C]{\thepage}
\renewcommand{\headrulewidth}{0.4pt}

% ----- Hiperligações -----
\usepackage[hidelinks]{hyperref}
\hypersetup{
  pdftitle={PayNexus: Uma Carteira Digital Distribuída em Microserviços},
  pdfauthor={Grupo 3 EGS - UA/DETI},
}

% ----- Referências -----
\usepackage[numbers,sort&compress]{natbib}
\bibliographystyle{unsrtnat}


% ============================================================
\begin{document}

\title{%
  \textbf{PayNexus: Uma Carteira Digital Distribuída em Microserviços}\\[0.5em]
  \large Relatório Técnico - Engenharia e Gestão de Serviços (EGS)
}
\author{
  Grupo 3 - DETI, Universidade de Aveiro\\[0.3em]
  Pedro Melo, nº 114208 (25\%)\\
  Simão Almeida, nº 113085 (25\%)\\
  Tiago Garcia, nº 114184 (25\%)\\
  Rúben Costa, nº 114190 (25\%)\\
  \small\textit{Engenharia de Computadores e Telemática}
}
\date{Junho de 2026}
\maketitle
\thispagestyle{fancy}

% ============================================================
\begin{abstract}
Este relatório descreve o projeto \textbf{PayNexus}, uma plataforma de carteira digital
implementada como um conjunto de microsserviços independentes. O problema central
abordado é o carregamento direto de dinheiro para a \emph{blockchain}: um utilizador
paga com cartão de crédito (Stripe), o pagamento é confirmado por \emph{webhook} com
verificação HMAC e protegido por código OTP via WhatsApp (Twilio), e o valor é
imediatamente creditado numa carteira Ethereum na testnet Sepolia via Web3j,
terminando com uma notificação \emph{Web Push} em tempo real. O sistema é composto por
seis serviços independentes (IAM, Pagamentos, Transações, Notificações e dois
\emph{frontends}) escritos em quatro linguagens (Python, Java, Go, TypeScript), com
base de dados própria por serviço (PostgreSQL~15 e Redis~7), \emph{gateway} Traefik~v3.1,
gestão centralizada de segredos com HashiCorp Vault~1.17, autenticação OAuth2/OIDC via
Keycloak~26.1 em dois \emph{realms}, e orquestração em Docker Compose (desenvolvimento)
e Kubernetes (produção no cluster DETI).
\end{abstract}

\tableofcontents
\newpage

% ============================================================
\section{Introdução}
\label{sec:introducao}

O projeto \textbf{PayNexus} nasce com o propósito de democratizar o acesso ao ecossistema 
\emph{blockchain}, eliminando as barreiras técnicas tradicionais de entrada no mundo Web3. 
A nossa proposta de valor centra-se na criação de uma ponte fluida e segura entre o sistema 
financeiro tradicional (Web2) e as redes descentralizadas (Web3), permitindo que qualquer 
utilizador carregue e movimente saldo \emph{on-chain} recorrendo simplesmente a um cartão de crédito.

Para viabilizar este modelo de negócio complexo — que exige a orquestração de domínios com 
ciclos de vida e modos de falha completamente distintos — afastámo-nos de uma abordagem 
monolítica tradicional. Em vez disso, o \textbf{PayNexus} foi desenvolvido com uma arquitetura baseada de \textbf{microsserviços} independentes. Cada serviço está isolado e foca-se num domínio específico do sistema, o que facilita a manutenção e reduz dependências entre componentes. Esta abordagem vai ao encontro dos objetivos curriculares da unidade de Engenharia e Gestão de Serviços (EGS): a arquitetura em si é o \emph{deliverable} principal, demonstrando princípios como \emph{bounded contexts}, implantação independente, desenvolvimento políglota, e
orquestração com contentores.

\textbf{Objetivos do projeto:}
\begin{itemize}[noitemsep]
  \item Implementar um fluxo completo de pagamento com cartão e crédito em \emph{blockchain};
  \item Demonstrar princípios de microserviços com tecnologias heterogéneas;
  \item Garantir segurança por camadas: autenticação, segredos em \emph{runtime}, isolamento de rede;
  \item Disponibilizar observabilidade operacional via dashboards Grafana;
  \item Orquestrar o sistema em Docker Compose (desenvolvimento) e Kubernetes (produção).
\end{itemize}

Este relatório está organizado da seguinte forma: a Secção~\ref{sec:visao} apresenta
uma visão geral do sistema; a Secção~\ref{sec:arquitetura} detalha a arquitetura de
microsserviços; a Secção~\ref{sec:stack} descreve a \emph{stack} tecnológica; a
Secção~\ref{sec:servicos} documenta a implementação de cada serviço; a
Secção~\ref{sec:comunicacao} analisa a comunicação inter-serviços; a
Secção~\ref{sec:seguranca} cobre os mecanismos de segurança; a
Secção~\ref{sec:containerizacao} descreve a containerização e orquestração; a
Secção~\ref{sec:observabilidade} aborda a observabilidade; e a
Secção~\ref{sec:conclusao} apresenta os desafios e conclusões.

% ============================================================
\section{Visão Geral do Sistema}
\label{sec:visao}

A plataforma PayNexus é uma \textbf{carteira digital distribuída}: os utilizadores
autenticam-se, carregam a carteira com um pagamento real por cartão, movem fundos como
transações \emph{blockchain}, e recebem notificações em tempo real, tudo entregue por
um conjunto de microsserviços independentes.

\begin{table}[H]
\centering
\caption{Resumo do projeto PayNexus}
\label{tab:glance}
\begin{tabularx}{\linewidth}{@{}L{4.5cm}X@{}}
\toprule
\textbf{Aspeto} & \textbf{Descrição} \\
\midrule
Domínio & Carteira digital / pagamentos / transações \emph{blockchain} \\
\rowsep
Arquitetura & Microserviços com \emph{gateway} único \\
\rowsep
Microserviços & \textbf{6} (4 \emph{backends} + 2 \emph{frontends}) \\
\rowsep
Linguagens & \textbf{4}Python, Java, Go, TypeScript \\
\rowsep
Autenticação & Keycloak (OAuth2/OIDC), \textbf{dois \emph{realms}} \\
\rowsep
Pagamentos & Stripe (PaymentIntent + \emph{webhooks}) + OTP Twilio \\
\rowsep
\emph{Blockchain} & Ethereum \textbf{Sepolia} testnet via Web3j \\
\rowsep
Tempo real & Notificações Web Push (multi-\emph{tenant}) \\
\rowsep
Dados & PostgreSQL 15 por serviço + Redis 7 \\
\rowsep
\emph{Gateway} & Traefik v3.1 (apenas porta 80 exposta) \\
\rowsep
Segredos & HashiCorp Vault (AppRole + \emph{agent sidecars}) \\
\rowsep
Observabilidade & Dashboards Grafana 10 \\
\rowsep
Desenvolvimento & Docker Compose (24 contentores) \\
\rowsep
Produção & Kubernetes (cluster DETI/UA) \\
\rowsep
Repositório & \url{https://github.com/Rubenc1234/EGS_project} \\
\rowsep
\emph{Vídeo demo} & \url{https://drive.google.com/file/d/1gJ6fvjBPGqH-XALS-913JTkQdW-j82iO/view?usp=sharing} \\
\bottomrule
\end{tabularx}
\end{table}

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{EGS_GERAL_ARCH.png}
  \caption{Arquitetura Geral do Sistema}
  \label{fig:arch}
\end{figure}

O roteamento do \emph{gateway} Traefik é baseado em \emph{headers} de \emph{host} HTTP,
conforme indicado na Tabela~\ref{tab:routing}.

\begin{table}[H]
\centering
\caption{Roteamento via Traefik (desenvolvimento local)}
\label{tab:routing}
\begin{tabularx}{\linewidth}{@{}L{5cm}L{5cm}C{2.5cm}@{}}
\toprule
\textbf{Host} & \textbf{Serviço} & \textbf{Porta interna} \\
\midrule
\texttt{app.pt}              & Frontend principal      & 80    \\
\rowsep
\texttt{payment.pt}          & Payment frontend        & 5174  \\
\rowsep
\texttt{iam.pt}              & IAM service             & 5000  \\
\rowsep
\texttt{transactions.pt}     & Transactions service    & 8081  \\
\rowsep
\texttt{payment-api.pt}      & Payment backend         & 5002  \\
\rowsep
\texttt{notifications.pt}    & Notifications service   & 8080  \\
\rowsep
\texttt{keycloak.pt}         & Keycloak (compositor)   & 8080  \\
\rowsep
\texttt{payment-keycloak.pt} & Keycloak (pagamentos)   & 8080  \\
\rowsep
\texttt{grafana.pt}          & Grafana                 & 3000  \\
\bottomrule
\end{tabularx}
\end{table}

% ============================================================
\section{Arquitetura de Microserviços}
\label{sec:arquitetura}

\subsection{Princípios Aplicados}

A arquitetura do PayNexus foi desenhada com os princípios fundamentais de microsserviços
em mente. A Tabela~\ref{tab:principios} descreve como cada princípio foi concretizado
no projeto.

\begin{table}[H]
\centering
\caption{Princípios de microsserviços e aplicação no projeto}
\label{tab:principios}
\begin{tabularx}{\linewidth}{@{}L{4cm}X@{}}
\toprule
\textbf{Princípio} & \textbf{Aplicação no PayNexus} \\
\midrule
\emph{Bounded contexts} & Cada serviço possui um domínio exclusivo: identidade, pagamentos, transações, notificações \\
\rowsep
Implantação independente & Cada serviço tem o seu próprio \texttt{Dockerfile}, imagem e ciclo de vida \\
\rowsep
Desenvolvimento políglota & Python (APIs web rápidas), Java/Spring (blockchain + tipagem forte), Go (notificações de alto débito) \\
\rowsep
Base de dados por serviço & Sem esquema partilhado; instância PostgreSQL isolada por serviço em rede própria \\
\rowsep
Isolamento de falhas & A falha de um serviço não compromete a disponibilidade global do sistema, graças ao isolamento entre serviços e redes independentes \\
\rowsep
Ponto de entrada único & O Traefik esconde a topologia interna atrás de uma única porta \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{Catálogo de Serviços}

A Tabela~\ref{tab:catalogo} apresenta o catálogo completo dos seis serviços que compõem
a plataforma.

\begin{table}[H]
\centering
\footnotesize
\setlength{\tabcolsep}{4pt}
\caption{Catálogo de microsserviços do PayNexus}
\label{tab:catalogo}
\begin{tabularx}{\linewidth}{@{}L{3.0cm}L{3.6cm}L{2.8cm}C{1.1cm}X@{}}
\toprule
\textbf{Serviço} & \textbf{Responsabilidade} & \textbf{Framework} & \textbf{Porta} & \textbf{Bibliotecas-chave} \\
\midrule
\path{iam_service}           & Registo, login e validação de \emph{tokens} via Keycloak & Python/Flask 3.1 & 5000 & requests, Flask \\
\rowsep
\path{payment_service}       & Pagamentos com cartão, OTP, recibos, cartões guardados   & Python/Flask 3.1 & 5002 & Stripe 14.4, Twilio, SQLAlchemy, ReportLab \\
\rowsep
\path{transactions_service}  & Carteiras, transações \emph{blockchain}, composição BFF  & Java 21 / Spring Boot 3.4.3 & 8081 & Web3j 4.10, Spring Data JPA, SpringDoc \\
\rowsep
\path{notifications_service} & Clientes, subscrições, Web Push em tempo real & Go/Gin & 8080 & gin, gorm, webpush-go, jwt \\
\rowsep
frontend                     & UI principal da carteira e histórico & React 18 + TypeScript + Vite & 80 & React Router, Axios, React Query, MUI \\
\rowsep
payment frontend             & UI de pagamento dedicada & React 18 + TypeScript + MUI & 5174 & @stripe/react-stripe-js, MUI \\
\bottomrule
\end{tabularx}
\end{table}

% ============================================================
\section{Stack Tecnológica}
\label{sec:stack}

A Tabela~\ref{tab:stack} resume todas as tecnologias utilizadas na plataforma, organizadas
por camada funcional.

\begin{longtable}{@{}L{3cm}L{4cm}L{8.1cm}@{}}
\caption{Stack tecnológica completa do PayNexus}
\label{tab:stack}\\
\toprule
\textbf{Camada} & \textbf{Tecnologia} & \textbf{Versão / Notas} \\
\midrule
\endfirsthead
\multicolumn{3}{l}{\small\textit{(continuação da Tabela~\ref{tab:stack})}}\\
\toprule
\textbf{Camada} & \textbf{Tecnologia} & \textbf{Versão / Notas} \\
\midrule
\endhead
\bottomrule
\endfoot
\emph{Frontend} & React + TypeScript + Vite & React 18.2, Vite 7.3, servido via nginx \\
\rowsep
 & Material-UI (MUI) & 7.3.9biblioteca de componentes UI \\
\rowsep
 & Axios, React Query, React Router & HTTP, \emph{data fetching}, roteamento \\
\midrule
\emph{Backend} & Python 3.11 + Flask 3.1 & IAM e Payment services \\
\rowsep
 & Java 21 + Spring Boot 3.4.3 & Transactions service \\
\rowsep
 & Go + Gin 1.10 & Notifications service \\
\midrule
Dados & PostgreSQL 15 & Uma instância por serviço \\
\rowsep
 & Redis 7 & Notificações - cache e pub/sub \\
\rowsep
 & ORMs & SQLAlchemy, Spring Data JPA, GORM \\
\midrule
Autenticação & Keycloak 26.1 & Dois \emph{realms}, OAuth2/OIDC, JWT \\
\midrule
\emph{Gateway} & Traefik v3.1 & Roteamento por \emph{host}, multi-instância \\
\midrule
Contentores & Docker + Docker Compose & \emph{Multi-stage builds} \\
\rowsep
Orquestração & Kubernetes & Cluster DETI, Traefik Ingress \\
\midrule
Segredos & HashiCorp Vault 1.17 & AppRole + \emph{agent sidecars} \\
\midrule
Observabilidade & Grafana 10 & Datasources PostgreSQL por serviço \\
\midrule
Pagamentos & Stripe SDK 14.4.1 & PaymentIntent + \emph{webhooks} \\
\rowsep
OTP/SMS & Twilio SDK 9.10.4 & WhatsApp OTP \\
\rowsep
\emph{Blockchain} & Web3j 4.10 + Ethereum Sepolia & Chain ID: 11155111 \\
\rowsep
\emph{Push} & Web Push API + webpush-go & VAPID por cliente \emph{tenant} \\
\end{longtable}

% ============================================================
\section{Implementação dos Serviços}
\label{sec:servicos}

\subsection{IAM Service (Serviço de Identidade)}

O \textbf{IAM Service} é o ponto de entrada para autenticação de utilizadores. Implementado
em Python/Flask~3.1, funciona como um \emph{proxy} OIDC que delega toda a lógica de
identidade ao Keycloak.

\textbf{Responsabilidades:}
\begin{itemize}[noitemsep]
  \item Gerir o fluxo de \emph{sign-up} e \emph{login} via Keycloak (\emph{realm} microservices);
  \item Trocar o código de autorização OAuth2 por \emph{tokens} JWT;
  \item Validar \emph{tokens} em pedidos subsequentes;
  \item Expor documentação Swagger em \texttt{/iam/docs}.
\end{itemize}

A integração com o protocolo OIDC/OAuth2 foi desenvolvida de raiz através de chamadas 
HTTP diretas aos endpoints do Keycloak, utilizando a biblioteca \texttt{requests}. 
O estado de sessãonecessário para a validação do parâmetro \texttt{state} do 
OAuth2é gerido de forma \emph{stateless} no cliente. O \emph{backend} em Flask 
assina criptograficamente os dados da sessão e armazena-os num \emph{cookie} seguro 
no navegador do utilizador, garantindo que o serviço permanece completamente 
independente de estado local e perfeitamente preparado para escalabilidade horizontal.

\subsection{Payment Service (Serviço de Pagamentos)}

O \textbf{Payment Service}, implementado em Python/Flask~3.1 com SQLAlchemy e
PostgreSQL~15, é responsável por todo o ciclo de vida dos pagamentos com cartão.

\textbf{Fluxo principal:}
\begin{enumerate}[noitemsep]
  \item O \emph{frontend} invoca \texttt{POST /v1/payments} para criar um \texttt{PaymentIntent} na Stripe;
  \item O Stripe.js no browser confirma o pagamento diretamente com a Stripe;
  \item A Stripe envia um \emph{webhook} com assinatura HMAC para \texttt{POST /v1/payments/webhook};
  \item O serviço verifica a assinatura no cabeçalho \texttt{Stripe-Signature};
  \item É enviado um OTP de 6 dígitos via WhatsApp (Twilio), com validade de 5 minutos;
  \item Após verificação do OTP, o pagamento é marcado como \texttt{concluded} e o serviço chama o Transactions Service (Tabela~\ref{tab:endpoints-payment}).
\end{enumerate}

\begin{table}[H]
\centering
\footnotesize
\setlength{\tabcolsep}{5pt}
\caption{Endpoints principais do Payment Service}
\label{tab:endpoints-payment}
\begin{tabularx}{\linewidth}{@{}>{\footnotesize\ttfamily\raggedright\arraybackslash}p{4.6cm}C{1.6cm}X@{}}
\toprule
\textbf{Endpoint} & \textbf{Método} & \textbf{Descrição} \\
\midrule
/v1/payments          & POST    & Cria PaymentIntent na Stripe \\
\rowsep
/v1/payments          & GET     & Lista pagamentos do utilizador (Bearer JWT) \\
\rowsep
/v1/payments/\{id\}   & PATCH   & Atualiza estado do pagamento \\
\rowsep
/v1/payments/\{id\}/send-otp & POST & Envia OTP via Twilio WhatsApp \\
\rowsep
/v1/payments/\{id\}/verify   & POST & Valida código OTP \\
\rowsep
/v1/payments/webhook  & POST    & Recebe \emph{webhook} Stripe (HMAC) \\
\rowsep
/v1/payments/\{id\}/receipt  & GET  & Gera recibo PDF (ReportLab) \\
\rowsep
/v1/users/profile     & GET/PUT  & Perfil do utilizador \\
\rowsep
/v1/users/cards       & POST/DELETE & Cartões guardados (Stripe Customer) \\
\bottomrule
\end{tabularx}
\end{table}

\textbf{Modelo de dados (PostgreSQL):} o serviço mantém quatro tabelas 
\texttt{payments} (UUID, user\_id, montante, estado, stripe\_payment\_intent\_id,
wallet\_id), \texttt{user\_profiles} (\allowbreak stripe\_customer\_id, número de telefone), 
\texttt{saved\_cards} (payment\_method\_id, last4, marca, validade) e
\texttt{payment\_otps} (código com hash SHA-256, expiração, flag de utilização).

\textbf{Geração de recibos PDF:} a biblioteca ReportLab gera recibos em memória
(\emph{in-memory}), com valores em EUR e tipografia Helvetica, servidos com
\texttt{Content-Disposition: attachment}.

\subsection{Transactions Service (Serviço de Transações e Blockchain)}

O \textbf{Transactions Service} é o núcleo central da plataforma. Implementado em
Java~21 com Spring Boot~3.4.3, integra-se diretamente com a \emph{blockchain} Ethereum e atua 
simultaneamente como motor de carteiras e como um \emph{API Composer/BFF} (Backend for Frontend), orquestrando o fluxo financeiro inter-serviços.

\textbf{Integração blockchain (Web3j~4.10):}
\begin{itemize}[noitemsep]
  \item \textbf{Rede:} Ethereum Sepolia testnet (chain ID: 11155111);
  \item \textbf{Carteiras:} Criadas por utilizador, com as chaves privadas armazenadas 
        e encriptadas em AES recorrendo a uma chave mestra (\texttt{MASTER\_KEY\_FOR\_WALLET} 
        injetada em \emph{runtime} via Vault);
  \item \textbf{Operações:} Consulta de saldos, estimativa de \emph{gas}, assinatura de 
        \emph{Raw Transactions} e consulta assíncrona de recibos de blocos (\emph{Receipts});
  \item \textbf{Modos:} \emph{Mock provider} para testes locais e \emph{real provider} para 
        integração efetiva com a rede de testes.
\end{itemize}

\textbf{Papel de Composer / BFF:} O serviço centraliza os pedidos do Frontend de Transações, 
atuando como agregador. Ele expõe endpoints para iniciar o fluxo de autenticação do operador 
(delegando as chamadas de \emph{login} e \emph{signup} ao \emph{Payment Service} em Python) 
e coordena a transição de estados: validação de liquidação em Euros $\rightarrow$ transferência 
\emph{on-chain} de fundos $\rightarrow$ notificação ao vivo do utilizador. As operações pesadas na \emph{blockchain} e as chamadas ao serviço de notificações são delegadas para execução assíncrona através de fios de execução (\emph{threads}) independentes geridos pela anotação \texttt{@EnableAsync}.

\textbf{Endpoints principais:}
\begin{itemize}[noitemsep]
  \item \texttt{POST /v1/payments} Encaminha a solicitação de criação de uma intenção de 
        pagamento para o \emph{Payment Service}; se o pagamento for imediatamente retornado como concluído, despoleta de imediato a transferência de fundos \emph{on-chain};
  \item \texttt{PATCH /v1/payments/\{id\}} Atua como o \emph{endpoint} de receção de 
        notificações internas; é invocado de forma assíncrona pelo \emph{Payment Service} (Python) após o sucesso do \emph{webhook} da Stripe, sendo protegido por um bypass de autenticação via cabeçalho \texttt{X-Internal-Key}. Ao receber este sinal, o serviço inicia a transferência de ETH a partir da \texttt{bankWallet} para a carteira do utilizador;
  \item \texttt{GET /v1/payments/\{id\}} Consulta o detalhe e o estado atualizado de um pagamento;
  \item \texttt{GET /v1/pay/login} e \texttt{GET /v1/pay/signup} Disponibilizam as URLs de redirecionamento OIDC geradas pelo ecossistema de pagamentos.
\end{itemize}

Para além dos \emph{endpoints} de orquestração (Composer/BFF) descritos acima, o serviço
expõe a API central de carteiras e transações resumida na Tabela~\ref{tab:endpoints-transactions}.

\begin{table}[H]
\centering
\footnotesize
\setlength{\tabcolsep}{5pt}
\caption{Endpoints principais do Transactions Service}
\label{tab:endpoints-transactions}
\begin{tabularx}{\linewidth}{@{}>{\footnotesize\ttfamily\raggedright\arraybackslash}p{5.6cm}C{1.4cm}X@{}}
\toprule
\textbf{Endpoint} & \textbf{Método} & \textbf{Descrição} \\
\midrule
/v1/users/me/wallet        & POST    & Cria a carteira \emph{on-chain} do utilizador \\
\rowsep
/v1/users/me/wallet        & GET     & Consulta a carteira do utilizador \\
\rowsep
/v1/wallet/init            & GET     & Inicializa/garante a carteira na primeira sessão \\
\rowsep
/v1/wallet/address         & GET     & Devolve o endereço público da carteira \\
\rowsep
/v1/wallets/\{id\}/\allowbreak balance & GET     & Consulta o saldo de uma carteira \\
\rowsep
/v1/transactions           & POST    & Cria uma transferência \emph{on-chain} \\
\rowsep
/v1/transactions           & GET     & Lista as transações do utilizador \\
\rowsep
/v1/transactions/\{id\}    & GET     & Consulta o detalhe de uma transação \\
\rowsep
/v1/transactions/refund    & POST    & Solicita o reembolso de uma transação \\
\rowsep
/v1/transactions/\allowbreak refund/\{id\}/\allowbreak accept & POST & Aceita um pedido de reembolso \\
\rowsep
/v1/transactions/\allowbreak refund/\{id\}/\allowbreak deny   & POST & Recusa um pedido de reembolso \\
\rowsep
/v1/blockchain/\{address\}/\allowbreak balance & GET & Consulta o saldo \emph{on-chain} de um endereço \\
\bottomrule
\end{tabularx}
\end{table}

\noindent Existe ainda um conjunto de \emph{endpoints} auxiliares sob o prefixo \texttt{/v1/dev/*}
(financiamento de carteiras, \emph{retry} de transações e consultas de saldo \emph{on-chain})
destinados exclusivamente a testes locais e não expostos em produção.

\subsection{Notifications Service (Serviço de Notificações)}

O \textbf{Notifications Service} é um motor \emph{Web Push} multi-\emph{tenant} de uso
geral, implementado em Go com o \emph{framework} Gin. Qualquer aplicação cliente pode ser
integrada em tempo de execução, recebendo uma chave API e par de chaves VAPID próprias.

\textbf{Ciclo de vida de um cliente \emph{tenant}:}
\begin{enumerate}[noitemsep]
  \item Administrador cria cliente via \texttt{POST /v1/admin/clients};
  \item Sistema devolve chave API + chaves VAPID (pública e privada);
  \item O cliente usa a chave VAPID pública para subscrever utilizadores no browser;
  \item Subscrições são registadas em \texttt{POST /v1/events/subscribe};
  \item O Transactions Service publica eventos em \texttt{POST /v1/events} com \emph{Bearer API key};
  \item O serviço entrega notificações cifradas via Web Push ao \emph{Service Worker} do browser.
\end{enumerate}

\textbf{Performance:} o Redis~7 é usado para cache de clientes e subscrições, reduzindo
consultas à base de dados PostgreSQL nas operações de alta frequência. Os \emph{tokens} JWT
de subscrição têm validade de 15 minutos.

\textbf{Segurança multi-tenant:} cada cliente recebe o seu próprio par de chaves VAPID,
garantindo isolamento criptográfico entre \emph{tenants} e cumprindo a RFC~8292~\cite{rfc8292}.

\subsection{Frontends (React)}

A plataforma inclui dois \emph{frontends} independentes:

\textbf{Frontend principal} (\texttt{app.pt}): SPA React~18 + TypeScript + Vite que
expõe a carteira digital, histórico de transações, e gestão de conta. Comunica com o
Transactions Service (BFF) para todas as operações. Servido por nginx Alpine.

\textbf{Payment Frontend} (\texttt{payment.pt}): SPA dedicada ao fluxo de pagamento,
construída com React~18 + MUI~7.3 e integra o Stripe.js via
\texttt{@stripe/react-stripe-js}. Isolada num Keycloak \emph{realm} separado, garantindo
que credenciais de utilizadores de pagamento nunca se misturam com o \emph{realm}
microservices.

A estratégia de \emph{containerização} dos \emph{frontends} seguiu duas abordagens distintas:

O \textbf{Transactions Frontend} utiliza um modelo \emph{multi-stage}. Na primeira fase, o código é compilado e, na fase final, os ficheiros estáticos são transferidos para um servidor \textbf{Nginx}, que os serve de forma leve no porto 80.

Por outro lado, o \textbf{Payment Frontend} corre diretamente em modo de desenvolvimento através do servidor nativo do \textbf{Vite} (NodeJS) no porto 5174, sem recurso ao Nginx. Ambos os serviços são geridos e encaminhados para o exterior pelo \textbf{Traefik} com base no domínio de acesso.

% ============================================================
\section{Comunicação Inter-Serviços}
\label{sec:comunicacao}

Todos os serviços comunicam via \textbf{REST/HTTP} sobre as redes internas Docker/Kubernetes.
São usados dois padrões de autenticação: \emph{tokens} \textbf{JWT Bearer} para chamadas
orientadas ao utilizador, e o cabeçalho \textbf{\texttt{X-Internal-Key}} para chamadas
de serviço-a-serviço que não devem expor credenciais de utilizador.

\begin{table}[H]
\centering
\caption{Chamadas inter-serviços no PayNexus}
\label{tab:interservice}
\begin{tabularx}{\linewidth}{@{}L{3.2cm}L{3.0cm}XL{3.0cm}@{}}
\toprule
\textbf{Chamada} & \textbf{De → Para} & \textbf{Endpoint} & \textbf{Autenticação} \\
\midrule
Concluir pagamento & Payment → Transactions & \texttt{PATCH /v1/pay\-ments/\{id\}} & \texttt{X-Internal-Key} \\
\rowsep
Emitir evento & Transactions → Notifications & \texttt{POST /v1/events} & \texttt{Bearer <api-key>} \\
\rowsep
Login/tokens & Frontend → IAM → Keycloak & Fluxo OIDC code & OAuth2 \\
\rowsep
Entrega Web Push & Notifications → Browser & Web Push cifrado & Chaves VAPID \\
\rowsep
\emph{Webhook} Stripe & Stripe → Payment & \texttt{POST /v1/pay\-ments/web\-hook} & HMAC \texttt{Stripe-Sig\-na\-ture} \\
\bottomrule
\end{tabularx}
\end{table}

Uma consequência arquitetural importante é a \textbf{consistência eventual}: um pagamento
só é marcado como ``concluído'' após o \emph{webhook} da Stripe ser recebido e
verificado  não quando o utilizador clica em ``pagar''. Isto significa que o sistema
aceita um período de latência entre a ação do utilizador e a atualização de estado,
em troca de garantia criptográfica de que o pagamento foi efetivamente processado.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{EGS_SEQ_ADD_FUNDS.png}
  \caption{Diagrama de Sequência de Fluxo de Pagamento e Carregamento de Carteira}
  \label{fig:seq-payment}
\end{figure}

% ============================================================
\section{Segurança}
\label{sec:seguranca}

A segurança do PayNexus é implementada em múltiplas camadas independentes, de forma a
que um eventual comprometimento de uma camada não exponha o sistema inteiro.

\subsection{Autenticação OAuth2/OIDC com Keycloak}

O Keycloak~26.1 é o provedor de identidade central da plataforma, configurado com
\textbf{dois \emph{realms} independentes}~\cite{keycloak,oauth2}:

\begin{itemize}[noitemsep]
  \item \textbf{\emph{Realm} microservices} (\texttt{keycloak.pt}): utilizado pela aplicação
        principal e pelo IAM Service para autenticar os utilizadores da carteira;
  \item \textbf{\emph{Realm} de pagamentos} (\texttt{payment-keycloak.pt}): utilizado
        exclusivamente pelo Payment Service, garantindo isolamento total entre os dois
        contextos de autenticação.
\end{itemize}

Esta separação implica que uma violação de credenciais no \emph{realm} de pagamentos não
compromete os utilizadores da carteira, e vice-versa. O custo é a duplicação de
configuração, mitigada pela exportação de \emph{realms} como ficheiros JSON versionados
(\texttt{realm-export.json}).

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{EGS_SEQ_LOGIN.png}
  \caption{Fluxo de Login OIDC (OAuth2 Authorization Code Flow)}
  \label{fig:seq-login}
\end{figure}

\subsection{Gestão de Segredos com HashiCorp Vault}

Nenhum segredo é codificado na aplicação ou incluído nas imagens Docker. O HashiCorp
Vault~1.17~\cite{vault-approle,vault-agent} é o repositório centralizado de segredos,
com o padrão \textbf{AppRole + \emph{agent sidecar}}:

\begin{enumerate}[noitemsep]
  \item Cada serviço tem um \texttt{vault-agent} dedicado com \texttt{role\_id} e
        \texttt{secret\_id} próprios (AppRole);
  \item O agente autentica-se no Vault ao arrancar e renderiza o template
        \texttt{vault/\allowbreak templates/\allowbreak <serviço>.env.tmpl};
  \item O ficheiro \texttt{.env} resultante é montado em volume partilhado com o serviço;
  \item O processo do serviço lê as variáveis de ambiente sem nunca aceder diretamente
        ao Vault.
\end{enumerate}

\begin{table}[H]
\centering
\small
\caption{Segredos geridos por serviço no Vault}
\label{tab:vault}
\begin{tabularx}{\linewidth}{@{}L{3.8cm}X@{}}
\toprule
\textbf{Serviço} & \textbf{Segredos injetados} \\
\midrule
\path{payment_service} &
  \texttt{STRIPE\_SECRET\_KEY}, \texttt{STRIPE\_WEBHOOK\_SECRET} \newline
  \texttt{TWILIO\_ACCOUNT\_SID}, \texttt{TWILIO\_AUTH\_TOKEN} \newline
  \texttt{DATABASE\_URL}, \texttt{INTERNAL\_API\_KEY}, credenciais Keycloak \\
\rowsep
\path{transactions_service} &
  \texttt{MASTER\_KEY\_FOR\_WALLET} (chave AES para carteiras) \newline
  \texttt{NOTIFICATIONS\_API\_KEY}, \texttt{APP\_INTERNAL\_API\_KEY} \newline
  \texttt{BLOCKCHAIN\_NODE\_URL} \\
\rowsep
\path{notifications_service} &
  \texttt{MASTER\_ADMIN\_SECRET}, \texttt{JWT\_SECRET}, \texttt{DATABASE\_URL} \\
\rowsep
\path{iam_service} &
  \texttt{CLIENT\_SECRET} (Keycloak) \\
\rowsep
\emph{Bases de dados} &
  Password PostgreSQL por cada instância (\path{payment_postgres}, \path{transaction_postgres}, \path{notifications_postgres}) \\
\bottomrule
\end{tabularx}
\end{table}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{EGS_Vault_diagram.png}
  \caption{Padrão Vault AppRole com Agent Sidecar}
  \label{fig:vault}
\end{figure}

\subsection{Outros Mecanismos de Segurança}

\begin{table}[H]
\centering
\caption{Mecanismos de segurança adicionais}
\label{tab:seguranca}
\begin{tabularx}{\linewidth}{@{}L{4.5cm}X@{}}
\toprule
\textbf{Mecanismo} & \textbf{Aplicação} \\
\midrule
RBAC (\emph{role} \texttt{operator}) & Endpoints de administração (estatísticas, criação de clientes) apenas acessíveis com papel \emph{operator} no JWT \\
\rowsep
Verificação HMAC \emph{webhook} & Cabeçalho \texttt{Stripe-Signature} verificado com a chave \texttt{STRIPE\_WEBHOOK\_SECRET} antes de processar qualquer evento \\
\rowsep
Encriptação AES de carteiras & Chaves privadas Ethereum encriptadas com \texttt{MASTER\_KEY\_FOR\_WALLET} antes de persistir na base de dados \\
\rowsep
OTP com hash SHA-256 & Código OTP armazenado apenas como hash; expiração de 5 minutos; flag \texttt{used} impede reutilização \\
\rowsep
Isolamento de rede & Redes de bases de dados com \texttt{internal: true}  inacessíveis pelo Traefik ou pela rede pública \\
\rowsep
VAPID por \emph{tenant} & Isolamento criptográfico entre clientes nas notificações Web Push \\
\bottomrule
\end{tabularx}
\end{table}

% ============================================================
\section{Containerização e Orquestração}
\label{sec:containerizacao}

\subsection{Docker Compose (Desenvolvimento)}

Em desenvolvimento, a plataforma é orquestrada com Docker Compose num ficheiro raiz que
define \textbf{24 contentores} (15 de aplicação/infraestrutura e 9 \emph{agentes Vault}),
4 redes e 6 volumes nomeados~\cite{docker-multistage}.
O \emph{frontend} principal e o serviço de notificações usam \emph{multi-stage builds};
os restantes correm em imagens \emph{single-stage}:

\begin{table}[H]
\centering
\small
\setlength{\tabcolsep}{5pt}
\caption{Imagens Docker por serviço e estratégias de \emph{build}}
\label{tab:docker}
\begin{tabularx}{\linewidth}{@{}L{3.5cm}XL{4.0cm}@{}}
\toprule
\textbf{Serviço / Componente} & \textbf{Imagem de Build} & \textbf{Imagem de Runtime} \\
\midrule
Transactions (Java) & --- (\emph{jar} pré-compilado) & \path{eclipse-temurin:21-jre} \\
\rowsep
Payment Service (Python) & --- & \path{python:3.11-slim} \\
\rowsep
Notifications (Go) & \path{golang:1.26-alpine} & \path{alpine} \\
\rowsep
Transactions Frontend (React) & \path{node:20-slim} & \path{nginx:alpine} \\
\rowsep
Payment Frontend (React) & --- & \path{node:20-slim} \\
\bottomrule
\end{tabularx}
\end{table}

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{EGS_REDE_DOCKER.png}
  \caption{Topologia de Redes Docker Compose}
  \label{fig:docker-net}
\end{figure}

\subsection{Kubernetes (Produção)}

Em produção, a plataforma corre no cluster Kubernetes do DETI/UA, no namespace
\texttt{tenant-grupo3-egs-deti-ua-pt}~\cite{kubernetes}. Ao contrário do encaminhamento
local por subdomínios, em produção todos os serviços são expostos a partir de um
\textbf{único \emph{host}} (\texttt{grupo3-egs-deti-ua.pt}) com \textbf{encaminhamento por
\emph{path}}, recorrendo a objetos \texttt{Ingress} padrão do Kubernetes anotados com
\texttt{ingressClassName: traefik} e terminação TLS (\texttt{secretName: transactions-tls}).
A única exceção é o Vault, exposto no subdomínio dedicado \texttt{vault.grupo3-egs-deti-ua.pt}.
A Tabela~\ref{tab:k8s-routing} resume o mapeamento de \emph{paths} e a Tabela~\ref{tab:k8s-manifests}
enumera os manifestos disponíveis.

\begin{table}[H]
\centering
\small
\setlength{\tabcolsep}{5pt}
\caption{Encaminhamento por \emph{path} no Ingress Traefik (produção)}
\label{tab:k8s-routing}
\begin{tabularx}{\linewidth}{@{}>{\ttfamily}L{3.6cm}>{\ttfamily\footnotesize}L{4.3cm}X@{}}
\toprule
\textbf{\normalfont Path} & \textbf{\normalfont Serviço (port)} & \textbf{\normalfont Descrição} \\
\midrule
/transactions      & transactions-frontend-svc:80   & \emph{Frontend} principal (carteira) \\
\rowsep
/transactions-api  & transactions-service:8081      & API/BFF de transações \\
\rowsep
/payment           & payment-frontend-svc:80        & \emph{Frontend} de pagamento \\
\rowsep
/payment-api       & payment-service:5002           & API de pagamentos \\
\rowsep
/iam               & iam-service:5000               & Serviço IAM \\
\rowsep
/notifications     & notifications-api:8080         & Serviço de notificações \\
\rowsep
/keycloak          & keycloak:8080                  & Keycloak (\emph{realm} microservices) \\
\rowsep
/payment-keycloak  & payment-keycloak:8080          & Keycloak (\emph{realm} pagamentos) \\
\rowsep
/grafana           & grafana:3000                   & Observabilidade \\
\rowsep
vault.\ldots/      & vault:8200                     & Vault (subdomínio próprio) \\
\bottomrule
\end{tabularx}
\end{table}

\begin{table}[H]
\centering
\small
\setlength{\tabcolsep}{5pt}
\caption{Manifestos Kubernetes disponíveis}
\label{tab:k8s-manifests}
\begin{tabularx}{\linewidth}{@{}L{4.8cm}X@{}}
\toprule
\textbf{Manifesto} & \textbf{Recursos definidos} \\
\midrule
\path{transactions-frontend.yaml} &
  Deployment (2 réplicas), Service ClusterIP, Ingress (\texttt{/transactions}) \\
\rowsep
\path{transactions-service.yaml} &
  Deployment da API + Deployment PostgreSQL com PVC (1\,Gi), ConfigMap, Secrets, Services, Ingress (\texttt{/transactions-api}) \\
\rowsep
\path{payment-frontend.yaml} &
  Deployment, ConfigMap, Service, Ingress (\texttt{/payment} e \texttt{/payment-api}) \\
\rowsep
\path{payment-service.yaml} &
  Deployment da API + Deployment PostgreSQL com PVC (1\,Gi), ConfigMap, Secrets, Services, Ingress (\texttt{/payment-api}) \\
\rowsep
\path{iam-service.yaml} &
  Deployment (2 réplicas), ConfigMap, Secret, Service, Ingress (\texttt{/iam}) \\
\rowsep
\path{notifications-service.yaml} &
  Deployment da API + Deployment PostgreSQL com PVC (1\,Gi) + Deployment Redis, ConfigMap, Secrets, Services, Ingress (\texttt{/notifications}) \\
\rowsep
\path{keycloak.yaml} &
  Deployment, ConfigMap (\emph{realm}), Secret, Service e dois Ingress (\texttt{/keycloak} HTTPS + \emph{redirect} HTTP$\rightarrow$HTTPS) \\
\rowsep
\path{payment-keycloak.yaml} &
  Deployment, ConfigMap (\emph{realm} de pagamentos), Secret, Service, Ingress (\texttt{/payment-keycloak}) \\
\rowsep
\path{vault.yaml} &
  Deployment, Service, Ingress (subdomínio \texttt{vault.\allowbreak grupo3-egs-deti-ua.pt}) \\
\rowsep
\path{grafana.yaml} &
  Deployment, 3 ConfigMaps (\emph{datasources}, \emph{providers}, \emph{dashboards}), Secret, Service, Ingress (\texttt{/grafana}) \\
\rowsep
\path{stripe-listener.yaml} &
  Deployment do \emph{listener} Stripe CLI para reencaminhar \emph{webhooks} (sem Ingress) \\
\rowsep
\path{*-tls-secret.yaml} &
  Três Secrets TLS (\texttt{transactions-tls}, \texttt{payment-tls}, \texttt{keycloak-tls}) com os certificados de terminação HTTPS \\
\bottomrule
\end{tabularx}
\end{table}

A Tabela~\ref{tab:compose-vs-k8s} compara as duas estratégias de orquestração.

\begin{table}[H]
\centering
\caption{Docker Compose vs.\ Kubernetes}
\label{tab:compose-vs-k8s}
\begin{tabularx}{\linewidth}{@{}L{3cm}L{4.5cm}L{4.5cm}@{}}
\toprule
\textbf{Aspeto} & \textbf{Docker Compose} & \textbf{Kubernetes} \\
\midrule
Uso & Desenvolvimento local e demonstrações & Produção (cluster DETI) \\
\rowsep
Escalamento & Manual & Declarativo via \texttt{replicas} \\
\rowsep
Roteamento & Contentor Traefik & Traefik Ingress Controller \\
\rowsep
Segredos & Vault agents $\to$ \texttt{.env} & K8s Secrets \\
\rowsep
Armazenamento & Volumes nomeados & PersistentVolumeClaims \\
\rowsep
Âmbito & 24 contentores, um \emph{host} & Namespace, distribuído por nós \\
\rowsep
Limites de recursos & Não definidos & CPU/memória por contentor \\
\bottomrule
\end{tabularx}
\end{table}

% ============================================================
\section{Observabilidade}
\label{sec:observabilidade}

A observabilidade da plataforma é garantida pelo \textbf{Grafana~10}~\cite{grafana},
disponível em \texttt{grafana.pt} (desenvolvimento) e
\texttt{grafana.grupo3-egs-deti-ua.pt} (produção).

\textbf{Datasources:} três instâncias PostgreSQL independentes são configuradas como
fontes de dados --- uma por serviço com dados (transações, pagamentos, notificações).
A ausência de uma base de dados partilhada obriga a que cada dashboard consulte a sua
própria instância, refletindo a separação de domínios da arquitetura.

\textbf{Dashboards disponíveis:}
\begin{itemize}[noitemsep]
  \item \texttt{main\_dashboard}: visão global do sistema --- transações realizadas,
        notificações enviadas, pagamentos concluídos;
  \item \texttt{payments\_dashboard}: volume de pagamentos, receita, taxa de sucesso,
        estatísticas OTP, padrões horários (12 painéis);
  \item \texttt{transactions\_dashboard}: métricas \emph{blockchain} --- transações
        on-chain, saldos, taxas de gas;
  \item \texttt{notifications\_dashboard}: entrega de notificações, falhas, canais ativos.
\end{itemize}

\textbf{Provisionamento:} todos os datasources e dashboards são declarados como
configuração (ficheiros YAML e JSON), sem necessidade de configuração manual na UI.
Em Kubernetes, este provisionamento é encapsulado em ConfigMaps, garantindo que
o Grafana arranca totalmente configurado.

% ============================================================
\section{Desafios e Conclusões}
\label{sec:conclusao}

\subsection{Desafios Encontrados}

\begin{table}[H]
\centering
\caption{Desafios e aprendizagens do projeto}
\label{tab:desafios}
\begin{tabularx}{\linewidth}{@{}L{4.5cm}X@{}}
\toprule
\textbf{Desafio} & \textbf{Aprendizagem} \\
\midrule
Operações políglota & 4 linguagens = 4 cadeias de build/runtime para containerizar e manter consistentes \\
\rowsep
Segredos à escala & Vault + \emph{agent sidecars} eliminou segredos do código, mas acrescentou componentes móveis ao sistema \\
\rowsep
Dois \emph{realms} Keycloak & Isolamento forte entre utilizadores da carteira e do pagamento, ao custo de duplicação de configuração \\
\rowsep
Pagamentos por \emph{webhook} & Confiar nos \emph{webhooks} da Stripe (não no cliente) obriga a abraçar a consistência eventual \\
\rowsep
\emph{Testnet} Ethereum & Transações reais na Sepolia requerem \emph{gas}, encriptação de chaves e um \emph{mock provider} para desenvolvimento \\
\rowsep
\emph{Gateway} único & O Traefik simplificou a superfície de ataque --- uma porta exposta, tudo o resto interno \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{Conclusões}

O projeto \textbf{PayNexus} demonstra com sucesso que é viável implementar um sistema \emph{fintech} com carregamento direto de dinheiro para a \emph{blockchain} usando uma arquitetura de microsserviços. A separação em \emph{bounded contexts} independentes permitiu usar a melhor ferramenta para cada domínio: Python para as APIs web, Java para a integração \emph{blockchain} com tipagem forte, e Go para o motor de notificações de alto débito.

A segurança por camadas, assente na federação de identidades, isolamento estrito de rede, validação de \emph{webhooks} e centralização de segredos, garante que nenhum vetor de ataque isolado compromete a integridade total do ecossistema, blindando os dados sensíveis dos utilizadores.

Em suma, a concretização deste projeto validou na prática os princípios fundamentais de Engenharia e Gestão de Serviços (EGS). Mais do que a simples integração tecnológica entre o mundo financeiro tradicional e as redes descentralizadas, o \textbf{PayNexus} provou que o verdadeiro valor de uma arquitetura políglota reside na capacidade de entregar resiliência, escalabilidade e independência operacional a cada serviço, resultando num produto final robusto, seguro e totalmente pronto para ambientes de produção.

% ============================================================
\bibliography{references}

\end{document}
